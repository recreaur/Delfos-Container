...
<!-- Modal confirmar abandonar repaso -->
<div id="confirmarAbandonarRepasoModal" class="modal hide" tabindex="-1" role="dialog" aria-labelledby="confirmarAbandonarRepasoModalLabel" aria-hidden="true">
    <div class="modal-header">
	<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
	<h4 id="confirmarAbandonarRepasoModalLabel"><s:text name="revision.exit.title"/></h4>
    </div>
    <div class="modal-body">
	<div class="texto"><s:text name="revision.exit.info"/></div>
    </div>
...
<script language="javascript">
	...
	$("#abandonar-repaso").click(function(event) {
	    event.preventDefault();
	    $("#confirmarAbandonarRepasoModal").modal();	    
	});
	...
	$("#ver-resumen").click(function() {
	    document.location = "resumenRepaso";
	});
    });
</script>
...
<div class="navbar-practica navbar navbar-googlenav">
	...
	<h5 class="navbar-text pull-left"><s:text name="revision"/></h5>
	<div class="btn-toolbar pull-right">
	    <div class="btn-group">		
		<button id="ver-resumen" class="btn"><span class="icon-list-alt"></span> <s:text name="summary.see"/></button>		
	    </div>
	    <div class="btn-group">
		<button id="abandonar-repaso" class="btn btn-danger"><s:text name="revision.exit.title"/></button>		
	    </div>
	</div>
    </div>
</div>
	...
	<div class="pagination exercises-pagination">
		...
		<s:iterator value="ejercicios" status="stat">	
			...
			<s:if test="alumnoEjerciciosRepaso.get(#stat.count).correcta">
			    <s:if test="alumnoEjerciciosRepaso.get(#stat.count).advertencias.empty">
			...
			<a id="ej<s:property value='#stat.count'/>" 
                           href="ejercicioRepaso?num=<s:property value='#stat.count'/>&anterior=<s:property value="num"/>" 
...
<div class="row top-buffer-compact">
	...
	<div class="caja">
		...
		<s:if test="alumnoEjerciciosRepaso.get(num)!=null">
		    <s:if test="alumnoEjerciciosRepaso.get(num).correcta">
	    ...
	    <div class="content">
		<s:textarea id="query-ejercicio" value="%{alumnoEjerciciosRepaso.get(num).respuesta}" theme="simple"/>
	    </div>
	    <div class="footer">
		<div class="pull-left">
		...
		<s:if test="num+1<=ejercicios.size()">
		    <ul class="pager pull-right">
                        <li class="next"><a href="ejercicioRepaso?num=<s:property value='num+1'/>&anterior=<s:property value="num"/>"><s:text name="exercise.next"/> <span class="icon-chevron-right"></span></a></li>
		    </ul>
		</s:if>
	    </div>
	</div>
	<s:iterator value="alumnoEjerciciosRepaso.get(num).advertencias" var="advertencia">
